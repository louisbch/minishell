...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/01 13:24:55 by lbouchon          #+#    #+#             */
/*   Updated: 2023/06/23 09:27:26 by mcourtin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	expander(t_token *token, t_env **my_env)
{
	...
	while (token->token_s[i])
	{
		...
		if (token->token_s[i] == '$' && token->token_s[i + 1] && \
		!is_space(token->token_s[i + 1]) && token->token_s[i + 1] != '"')
		{
